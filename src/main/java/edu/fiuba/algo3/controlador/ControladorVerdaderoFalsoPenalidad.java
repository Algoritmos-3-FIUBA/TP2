...
public class ControladorVerdaderoFalsoPenalidad extends Controlador {
    ...
    public void actualizarPlantilla(Pregunta pregunta, Jugador jugadorActual,TurnoJugador turnoActual) {
        ...
        for (int i = 0; i < pregunta.getOpciones().cantidadElementos(); i++) {
            cajasOpcionesMostradas.get(i).setOnAction(new SeleccionarRadioButtonHandler((PreguntaVerdaderoFalsoPenalidad) pregunta,opcionSeleccionada,cajasOpcionesMostradas.get(i).getText()));
